...
const DetailDelivery: React.FC<DetailDeliveryProps> = ({ reference }) => {
    ...
    useEffect(() => {
        const initBlockchain = async () => {
            ...
            if (accounts.length > 0) {
                ...
                const Orders = tabOrders.filter((x: { orderReference: any; }) => x.orderReference == reference);
                setOrders(Orders);
                ...
                setOrderProducts(tabOrderProducts);
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {
                                    orders?.map((order: any, index: any) => (
                                        orderProducts?.filter((x: { orderId: any; }) => x.orderId == order.id)
                                            .map((orderProduct: any) => (
                                                products?.filter((y: { id: any; }) => y.id == orderProduct.productId)
                                                    .map((product: any) => (
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td className="py-2 px-3 text-md text-gray-700 whitespace-nowrap text-center">{orderProduct.quantity.toString()}</td>
                                                            <td className="py-2 px-3 text-xl font-semibold bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 bg-clip-text text-transparent whitespace-nowrap text-center">
                                                                {product.price.toString() * orderProduct.quantity.toString()} <span className='font-normal text-xs text-gray-600'>$</span>
                                                            </td>
